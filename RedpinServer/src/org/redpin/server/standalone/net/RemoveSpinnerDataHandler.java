/**
 *  Filename: RemoveSpinnerDataHandler.java (in org.redpin.server.standalone.net)
...
public class RemoveSpinnerDataHandler implements IHandler {

	
	SpinnerDataHome spindataHome;
	
	public RemoveSpinnerDataHandler() {
	...
	public Response handle(JsonElement data) {
		...
		if(spindataHome.remove(sdata)) {
			res = new Response(Status.ok, null, null);
			Log.getLogger().finer("removed spinnerdata from database");
		} else {
			res = new Response(Status.failed, "could not remove from database", sdata);
			Log.getLogger().fine("could not remove spinnerdata from database");
		}
