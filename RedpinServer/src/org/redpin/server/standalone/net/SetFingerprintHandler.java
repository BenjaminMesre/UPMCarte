...
public class SetFingerprintHandler implements IHandler {
	...
	public Response handle(JsonElement data) {
		...
		if (fprint.getLocation() != null && ((Location)fprint.getLocation()).getId() != null && ((Location)fprint.getLocation()).getId().intValue() != -1) {
			Location l = HomeFactory.getLocationHome().getLocation(((Location)fprint.getLocation()).getId(), null, null, null, null, null, null);
		...
		if(fprint == null) {
			...
			if (count < INSTANT_TRAIN_THREASHOLD) {
				Log.getLogger().fine("Training model (fp count for loc " + loc.getSymbolicID() + loc.getBatimentID()+ loc.getTypeSalleID()
																	+ loc.getSalleID() + loc.getOpeningTIME()+ loc.getClosingTIME()+ ": " + count);
