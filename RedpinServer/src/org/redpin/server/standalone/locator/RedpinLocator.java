	...
	public Location locate(Measurement currentMeasurement) {
		...
		for(Measurement m : list) {
			...
			if(debug) {
				...
				if(fp != null) {
					...
					if(l != null) {
						log.log(Level.FINE, "location \"" + l.getMap().getMapName() + l.getSymbolicID()+
														l.getBatimentID()+ l.getTypeSalleID()+ l.getSalleID()
															+l.getOpeningTIME()+ l.getClosingTIME() + "\" achieved similarity level " + level);
					}
				}			
				//end debug
			}
		...
		if (hits.size() > 0) {
			...
			if(f != null) {
				...
				if(debug) {			
					log.log(Level.FINE, "Best match: \"" + loc.getMap().getMapName() + loc.getSymbolicID() +
							loc.getBatimentID() + loc.getTypeSalleID()+ loc.getSalleID() 
							+ loc.getOpeningTIME() + loc.getClosingTIME() + "\" achieved similarity level " + loc.getAccuracy());			
				}
			}
	...
	public int measurementSimilarityLevel(org.redpin.base.core.Measurement t,
		...
		if(debug) {
			...
			if(mloc != null) {
				log.log(loglevel, "Calculating similarity Level between current measurement and " + mloc.getMap().getMapName() + mloc.getSymbolicID() +
															mloc.getBatimentID() +mloc.getTypeSalleID()+ mloc.getSalleID()+ mloc.getOpeningTIME()+ mloc.getClosingTIME());
			} else {
		...
		String logmsg = "Comparing measurements..."
			+ "\n" + "-> Testing Measur: " + ( mloc == null ? ((Measurement)t).getId() : mloc.getMap().getMapName() + mloc.getSymbolicID()
																					+ mloc.getBatimentID()+mloc.getTypeSalleID() 
																					+ mloc.getSalleID() + mloc.getOpeningTIME()+ mloc.getClosingTIME())
