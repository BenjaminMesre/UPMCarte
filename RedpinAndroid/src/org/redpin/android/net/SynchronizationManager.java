...
public class SynchronizationManager extends Service {
	...
	private void sync() {
		...
		SpinnerDataRemoteHome
		.getSpinnerDataList(new RemoteEntityHomeCallback() {

			@Override
			public void onResponse(Response<?> response) {
				isSynced = true;
				Log.v(TAG, "database SpinnerData synchronized");
				syncInProgress = false;
			}
			...
			public void onFailure(Response<?> response) {
				Log.v(TAG,
						"database SpinnerData location synchronized failed: "
								+ response.getMessage());
				syncInProgress = false;
			}
		});
